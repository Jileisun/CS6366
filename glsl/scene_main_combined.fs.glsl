...
vec3 SkyCol(vec3 ro, vec3 rd) {
  ...
  return mix(
    SkyBg(rd) + 0.45 * pow(max(dot(rd, sunDir), 0.0), 16.0),
    vec3(0.83, 0.84, 0.87),
    clamp(0.75 * f * rd.y + 0.1, 0.0, 1.0)
  );
}
...
vec3 apolloTreesWarp(vec3 p) {
  p.x = abs(p.x) - uApolloTreeGap;
  p.y -= uApolloTreeLift + 0.35 * sin(0.07 * p.z + 0.6 * iTime);
  p.z *= 0.8;
  return p * uApolloTreeScale;
}
...
float apollonian(vec3 p, out float ss) {
  float s = mix(
    1.4,
    1.45,
    smoothstep(0.9, 1.0, sin(-length(p.xz) + TAU * iTime * uApolloTreeDrift * 0.1))
  );
  float scale = 1.5;
  const int rep = 5;
  mat2 rr = ROT(PI / 5.5);
  ...
  for (int i = 0; i < rep; ++i) {
    ...
    p.xz *= rr;
    float r2 = max(dot(p, p), 0.18);
    float k = s / r2;
    p *= k;
    scale *= k;
  }

  float d = -(box(p - 0.1, vec3(1.0, 2.0, 1.0)) - 0.6);
  ss = scale;
  return 0.25 * d / scale;
}

vec3 ApolloGlowWorld(vec3 ro, vec3 rd, float tMax) {
  float t = 1.0;
  vec3 col = vec3(0.0);
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    vec3 p = apolloTreesWarp(pWorld);
    float ss;
    float dist = apollonian(p, ss);
    if (dist > 20.0) {
      t += 2.0;
      continue;
    }

    float lateral = smoothstep(1.0, 5.0, abs(pWorld.x) - 2.0);
    float haze = exp(-0.08 * t);
    vec3 band = 0.5 + 0.5 * cos(3.0 + vec3(1.0, 2.0, 3.0) + 0.8 * log(max(ss, 1.0)));
    vec3 glow = mix(vec3(0.14, 0.34, 0.78), vec3(0.94, 0.54, 0.18), band);
    float shadeDist = max(dist, 0.0);
    col += 0.05 * glow * exp(-0.5 * shadeDist * float(i * i)) * haze * lateral;

    if (dist < 0.00035 * t) break;
    t += clamp(dist, 0.09, 1.8);
  }

  return col * uApolloTreeGain;
}
...
vec3 ComposeScene(vec3 ro, vec3 rd) {
  ...
  if (dstGrnd < dstFar) {
    ...
    col = mix(
      mix(vec3(0.19, 0.31, 0.16), vec3(0.11, 0.27, 0.18), f),
      mix(vec3(0.32, 0.22, 0.18), vec3(0.37, 0.28, 0.31), f),
      smoothstep(1.0, 3.0, hit.y)
    );
    col = mix(vec3(0.38, 0.28, 0.18), col, smoothstep(0.2, 0.6, abs(vn.y)));
    col = mix(col, vec3(0.76, 0.72, 0.74), smoothstep(5.0, 8.0, hit.y));
    col = mix(col, vec3(0.92), smoothstep(7.0, 9.0, hit.y) * smoothstep(0.0, 0.5, abs(vn.y)));
    float spec = mix(0.1, 0.5, smoothstep(8.0, 9.0, hit.y));
    vn = VaryNf(2.0 * hit, vn, 1.5);
    float sh = (shType > 1) ? GrndSShadow(hit, sunDir) : 1.0;
  ...
  float apolloLimit = (dstGrnd < dstFar) ? dstGrnd : dstFar;
  col += ApolloGlowWorld(ro, rd, apolloLimit);
